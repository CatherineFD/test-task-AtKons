...
export default defineComponent({
  ...
  setup() {
    ...
    let tableData: { message: string }[];

    if (matches) {
      const dataArray = text.split(regex).filter(Boolean);
      const result = matches.map((time, index) => time + (dataArray[index] || '')).concat((matches.pop() || '') + (dataArray.pop() || ''));

      tableData = result.map(item => ({ message: item.trim() }));
